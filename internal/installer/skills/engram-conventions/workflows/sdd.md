...
In interactive review mode — when the user wants to validate requirements before
the design phase begins — emit the review URL after saving:
...
When the user is expected to review the design before the next phase, emit the
review URL so they can validate the approach before apply begins:
...
```
# Goal: resume a change — get all phases at once
...
# Goal: all reports (verify + archive + apply progress) under sdd/
# MCP doesn't expose topic_key_prefix; fetch broadly + post-filter:
mem_search({"type": "report", "project": "myapp"})
# Then in your code: filter results where topic_key.startswith("sdd/")
```
...
```
# Goal: all frontend SDD artifacts for auth-refactor
# MCP doesn't expose topic_key_prefix; fetch broadly + post-filter:
mem_search({"query": "auth-refactor", "project": "myapp"})
# Then in your code: filter results where topic_key.startswith("frontend/sdd/auth-refactor/")

# Goal: all SDD artifacts for auth-refactor across all repos
# MCP doesn't expose topic_key_prefix; fetch broadly + post-filter:
mem_search({"query": "auth-refactor", "project": "myapp"})
# Then in your code: filter results where topic_key.startswith("sdd/auth-refactor/")
